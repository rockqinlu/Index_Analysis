...
sql_fz_1 = "SELECT X.INDIC_KEY, X.INDIC_NAME, X.STAT_DT, X.IND_VAL FROM t09_rm_indic X " \
         "WHERE 1=1  AND X.INDIC_TYPE = \'1\' " \
         "AND X.STAT_DT <= DATE("+p_stat_dt+")  " \
         "AND X.ORG_NUM = "+p_org + " " \
         "AND X.CURR_CD = " + p_curr_cd + "  " \
         "ORDER BY STAT_DT,INDIC_KEY "
sql_fz_2 = "SELECT X.INDIC_KEY, X.INDIC_NAME, X.STAT_DT, X.IND_VAL FROM t09_rm_indic X " \
         "WHERE 1=1  AND X.INDIC_TYPE = \'2\' " \
         "AND X.STAT_DT <= DATE("+p_stat_dt+")  " \
         "AND X.ORG_NUM = "+p_org + " " \
         "AND X.CURR_CD = " + p_curr_cd + "  " \
         "ORDER BY STAT_DT,INDIC_KEY "
sql_fz_3 = "SELECT X.INDIC_KEY, X.INDIC_NAME, X.STAT_DT, X.IND_VAL FROM t09_rm_indic X " \
...
# print('---------------------------- excel输出 ----------------------------------------------------------------')
with pd.ExcelWriter('D:\\test\\path_to_file4.xlsx') as writer:
